                ...
                <div class="">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <hr />
                            <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#tambahBarang"><i
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="no_transaksi" class="form-control"
                                                placeholder="No. Transaksi" readonly value="NT-00{{ time() }}">
                                        </div>
                                        <div class="form-group">
                                            <label for="tgl_transaksi">Tanggal Transaksi</label>
                                            <input type="text" name="tgl_transaksi" class="form-control"
                                                placeholder="Tanggal Transaksi" readonly value="{{ date('Y-m-d') }}">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label for="uang_pembeli">Uang Pembeli</label>
                                            <input type="text" name="uang_pembeli" id="uangPembeli" class="form-control"
                                                placeholder="Uang Pembeli" required>
                                        </div>
                                        <div class="form-group">
                                            <label for="kembalian">Uang Kembalian</label>
                                            <input type="text" value="" id="kembalian" name="kembalian"
                                                class="form-control" placeholder="Uang Kembalian" required readonly>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <select class="form-control" name="id_barang" id="barang-dropdown" required>
                            ...
                            @foreach ($barang as $item)
                                <option value="{{ $item->id }}" data-stok="{{ $item->stok }}"
                                    data-harga="{{ $item->harga }}">{{ $item->nama_barang }}</option>
                            @endforeach
                        </select>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('barangForm').addEventListener('submit', function(e) {
                ...
                row.innerHTML =
                    `<td>${itemCount}</td><td>${nama_barang}</td><td>${stokInput.value}</td><td>${harga}</td><td>${qty}</td><td>${subtotal}</td>`;
